...
### Database Changes ###
Database changes are treated a little differently to other code changes to assist their easy application to non-local-dev-environment databases on application startup using [dbup](https://github.com/DbUp/DbUp) . In addition to the small comment at the start of the file to indicate whether it has been manually modified or not, the database files are generated in a child directory of the project's data directory (which you control via config). You can name these child directories anything you like but skeleton will place the files it is generating in the "last" folder (based on an alphabitic sort of the child directory names). One useful convention is to name these directories '0001', '0002', '0003' etc. If you are making a schema change place your .sql files in '0003' and skeleton will place the code-generation changes to go along with the schema change in sub-directories along side it. Once these changes have been merged create a new child directory '0004' for new changes to be placed in. 

...
1. Re-generate your application from the Skeleton.Console directory (the same location as step 7 above) by running `skeleton-codegen -u -react -c survey -del`
...
```
  ...
  -r, --root=VALUE           the root folder to generate code into.
      ...
      --react-native         Generate a React Native client for application
...
### Database Changes ###
Database changes are treated a little differently to other code changes to assist their easy application to non-local-dev-environment databases on application startup using [dbup](https://github.com/DbUp/DbUp) . In addition to the small comment at the start of the file to indicate whether it has been manually modified or not, the database files are generated in a child directory of the project's data directory (which you control via config). You can name these child directories anything you like but skeleton will place the files it is generating in the "last" directory (based on an alphabitic sort of the child directory names). One useful convention is to name these directories '0001', '0002', '0003' etc. If you are making a schema change place your .sql files in '0003' and skeleton will place the code-generation changes to go along with the schema change in sub-directories along side it. Once these changes have been merged create a new child directory '0004' for new changes to be placed in. 
...
- noEdit: true|false - suppress creation of Edit operation for that type. Defaults to false.
- noSelectAll: true|false - suppress creation of "select all" and "select all by" functions. Defaults to false.

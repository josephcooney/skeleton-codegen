...
public class LinkingField : IJoiningField
{
    ...
    public string SelectExpression => $"array(select {_domain.TypeProvider.EscapeSqlName(_field.Type.Name)}.{_domain.TypeProvider.EscapeSqlName(_field.Name)} from {_domain.TypeProvider.EscapeSqlName(_field.Type.Namespace)}.{_domain.TypeProvider.EscapeSqlName(_field.Type.Name)} where {_domain.TypeProvider.EscapeSqlName(_currentTypeField.Name)} = {_alias}.{_domain.TypeProvider.EscapeSqlName(_currentTypeField.ReferencesTypeField.Name)}) as {_domain.TypeProvider.EscapeSqlName(Name)}";
}